...
class _RegisterFormScreenState extends State<RegisterFormScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isDeveloper ? 'Developer registration form' : 'Company registration form'),
      ),
      body: Padding(
                          ...
                          TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              suffixIcon: IconButton(
                                ...
                                onPressed: () {
                                  setState(() {
                                  _isPasswordVisible = !_isPasswordVisible; // Alternar la visibilidad de la contraseña
                                  });
                                },
                              ),
                            ),
                                ...
                                if (formKey.currentState!.validate()) {
                                  ...
                                  if(widget.isDeveloper){
                                    Developer developer = Developer(
                                      ...
                                      role: role, description: description, image: image,
                                    ...
                                    Company company = Company(
                                      ...
                                      phone: phone, password: password, role: role, description: description,
                                      image: image, bannerImage: bannerImage, ruc: ruc, owner: owner,
                                      name: name, address: address, country: country, city: city,
                                    );
  ...
  void verifyDataToRegister(){
    image = 'https://www.kindpng.com/picc/m/144-1447559_profile-icon-missing-profile-picture-icon-hd-png.png';
    if(widget.isDeveloper){
      bannerImage = 'https://www.techyon.es/media/news/web-developer-principales-responsabilidades-y-competencias_1650467356_100.jpg';
      role = 'DEVELOPER';
      description = 'I am a developer';

    }else{
      role = 'COMPANY';
      bannerImage = 'https://i0.wp.com/www.iedunote.com/img/23559/what-is-a-company-scaled.jpg?fit=2560%2C1696&amp;quality=100&amp;ssl=1';
      owner = 'Bill Gates';
      ruc = '123456789012345';
  ...
  Future registerDeveloper(Developer dev) async {
     final user = await LoginService.GetUserByEmail(email);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: FlashIncorrectMessageWidget(
                                message: 'Email already regystered'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
                           const SnackBar(
                            content: FlashCorrectMessageWidget(
                                message: 'Regystered successfully'),
                            behavior: SnackBarBehavior.floating,
                            backgroundColor: Colors.transparent,
                            elevation: 0.0,
                          ),
                        );
      final devCreated = await DeveloperService.getDeveloperById(response['id'].toString());
      ...
      print('-----------------------Digital Profile Instanced-----------------------');
      print(digitalProfile.toJson());
      final respondeDigitalProfile = await DeveloperService.setDigitalProfile(digitalProfile);
      print('-----------------------Digital Profile Inserted-----------------------');
      print(respondeDigitalProfile);
      //Cear sus dems cosas
      final createdDigitalProfile = await DeveloperService.getDigitalProfileByDeveloperId(response['id'].toString());
      print('-----------------------Digital Profile Created-----------------------');
      ...
      print('-----------------------Education Instanced-----------------------');
      print(education.toJson());

      final responseEducation = await DeveloperService.setEducationPublish(education);
      print('-----------------------Education Inserted-----------------------');
      print(responseEducation);

      Navigator.push(context, MaterialPageRoute(builder: (context) => MainDeveloperPage(developerId: response['id'].toInt())));
    }
  }
  ...
  Future registerCompany(Company company) async {
    final user = await LoginService.GetUserByEmail(email);
      ...
      Navigator.push(context, MaterialPageRoute(builder: (context) => MainCompanyPage(companyId: user2.id!)));
